...
namespace usagi
{
  namespace chrono
  {
    ...
    auto to_string_iso8601_gmt ( const TIME_POINT& t = typename TIME_POINT::clock::now() )
    -> std::string
    {
    ...
    template < typename TIME_POINT = std::chrono::steady_clock::time_point >
    auto to_string_iso8601_jst ( const TIME_POINT& t = typename TIME_POINT::clock::now() )
    {
      using namespace std::chrono;
    ...
    template < typename TIME_POINT = std::chrono::steady_clock::time_point >
    [[deprecated("to use: to_string_iso8601_gmt")]] auto to_string_iso8601 ( const TIME_POINT& t = typename TIME_POINT::clock::now() )

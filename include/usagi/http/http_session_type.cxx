...
{
  auto http_session_type::execute_handler() -> void
  {
    if ( only_localhost and socket.remote_endpoint().address().to_string() != "127.0.0.1" )
    {
      constexpr auto http_status_code_forbidden = static_cast< std::uint16_t >( 403 );
      write_error( http_status_code_forbidden );
      return;
    }
    
    const auto path_to_handler =
      shared_path_to_handler_mapper->find( headers.path );
  ...
  auto http_session_type::write_error( const std::uint16_t status_code ) -> void
  {
    ...
    , [ this_ = shared_from_this(), shared_string_buffer ]
      ...
      {
        //if ( e )
        //  std::cerr << "http server error: " << e.message() << std::endl;
        //else
  ...
  auto http_session_type::read_body() -> void
  {
    const auto entity =
  ...
  auto http_session_type::read_next_line() -> void
  {
    ...
    , [ this_ = shared_from_this() ]
      ( const boost::system::error_code& e, std::size_t )
      {
        ...
        // 続きは body で content_length > 0 なら読む
  ...
  auto http_session_type::read_first_line() -> void
  {
    boost::asio::async_read_until
    ( socket
    , buffer
  ...
  http_session_type::http_session_type
  ...
  auto http_session_type::interact() -> void
  {
    read_first_line();
  }
